defmodule ExCheck.ProjectCases.FailFastTest do
  ...
  test "--fail-fast escalates to SIGKILL when tool ignores SIGTERM", %{project_dir: project_dir} do
    config_path = Path.join(project_dir, ".check.exs")
    ...
    File.write!(
      ...
      [
        ...
        tools: [
          {:fail_tool, ["elixir", "-e", ":timer.sleep(150); IO.puts(\\"fail\\"); System.halt(1)"]},
          {:stubborn_tool, ["sh", "-c", "trap '' TERM; echo stubborn-start; sleep 30"]}
        ]
      ]
      """
    )
